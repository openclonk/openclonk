...
public func InitializePlayer(int plr, x, y, base, team, script_id)
{
	...
	if (this != possible_startpoints[0])
	{
		return false;
	}
	...
	if (n > 1)
	{
		while (possible_startpoints[n_best].penalty == best_penalty)
		{
			if (++n_best == n)
			{
				break;
...
{
	if (GetType(clonk_max_contents_count))
	{
		crew->~SetMaxContentsCount(clonk_max_contents_count);
	}
