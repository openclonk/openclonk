...
{
	Log("OnContentsSelection symbol: %d", symbol->GetName());
...
func FxIntRefreshContentsMenuTimer(target, effect, time)
{
	...
	if (GetLength(inventory) == GetLength(effect.last_inventory))
	{
		...
		{
			if (inventory[i].symbol == effect.last_inventory[i].symbol) continue;
			same = false;
			break;
		}
		if (same) return 1;
...
func DoMenuRefresh(int slot, int menu_index, array new_entries)
{
	...
	{
		var needs_insert = current_index >= GetLength(current_entries);
		var new_entry = new_entries[c];
		
		if (!needs_insert) // if an old entry exists, check equality
		{
			var old_entry = current_entries[current_index];
			
			if (EntriesEqual(new_entry, old_entry)) // already correct?
			{
				++current_index;
				continue;
			}
			
			// check if the entry appears later in the array (and is thus already shown)
			var found = false;
			for (var oi = current_index + 1; oi < GetLength(current_entries); ++oi)
			{
				if (!EntriesEqual(current_entries[oi], new_entry)) continue;
			...
			// check whether the current_entry was just removed
			found = false;
			for (var ni = 0; ni < GetLength(new_entries); ++ni)
			{
				if (!EntriesEqual(new_entries[ni], old_entry)) continue;
				
				found = true;
				break;
			}
			// we need to remove the old entry since it is not contained in new_entries?
			if (!found)
				menu.menu_object->RemoveItem(old_entry.unique_id, current_main_menu_id);
		}
		
		// do we need to insert the new entry?
		if (needs_insert)
		{
