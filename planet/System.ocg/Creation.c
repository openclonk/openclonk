	...
	if (mat != -1)
		while (i < amount)
		{
			...
			if (GetMaterial(rndx, rndy) == mat)
			{
				...
				obj = CreateObjectAbove(id, rndx, rndy + objhgt / 2, nil);
				obj->SetR(Random(360));
	...
	for (var i = plant_size; i < width; i += plant_size)
	{
		...
		else if (!Random(10) && GetLength(plants) > 1)
		{
			...
			for (j = 0; j < count; j++)
			{
				...
				if (y_pos == 0) continue;
				plant = CreateObjectAbove(plants[variance], x + i + spot, y_pos + 5, nil);
			}
			continue;
		}
		// No ground at y_pos?
		...
		plant = CreateObjectAbove(plants[0], x + i + x_variance, y_pos + 5, nil);
		plant->SetCon(growth);
